...
const SHOPIFY_STORE_PERMANENT_DOMAIN = "390115.myshopify.com";
const SHOPIFY_STOREFRONT_URL = `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}/api/${SHOPIFY_API_VERSION}/graphql.json`;
const SHOPIFY_STOREFRONT_TOKEN = "2f59da01fb706ebeac97e9887c80418b";
...
export async function storefrontApiRequest(query: string, variables: Record<string, unknown> = {}) {
  ...
  if (response.status === 402) {
    toast.error("Shopify: betaling vereist", {
      description: "Je Shopify-winkel heeft een actief abonnement nodig.",
    });
    return;
  }
  ...
  if (data.errors) {
    throw new Error(`Shopify fout: ${data.errors.map((e: { message: string }) => e.message).join(", ")}`);
  }
  return data;
}
...
// Yarn brand extraction from product title (e.g. "Comfy - 203 - Light Pink" → "Comfy")
export function extractYarnBrand(title: string): string {
